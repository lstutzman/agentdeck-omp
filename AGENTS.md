...
Daily install (2026-09-08): `~/.omp/agent/extensions/agentdeck-omp` is a symlink to this repository's `src/`. OMP discovers `extensions/<dir>/index.ts` through symlinks (`loader.ts` `discoverExtensionsInDir`), so every plain `omp` launch loads the working tree without a build or copy step; sessions started before the symlink existed do not have it until restarted. The smoke scripts still pass `--no-extensions --extension src/index.ts`. Without the symlink no session ever registers and the Stream Deck shows the plugin's empty placeholder (`HUB READY / NO SESSION / AgentDeck IDLE`, inert status cards from `session-slot-manager.ts`). Verify registration through a `client_register` companion socket and the `sessions_list` frame; there is no HTTP `/sessions` route.
